use origin_core::{
    ...
    verify_against_key, verify_chain_against_key, SecretKey,
};
...
#[test]
fn test_verify_against_key_correct_key() {
    ...
    let result = verify_against_key(&encoded, data, &tk);
    assert!(result.is_ok(), "correct trusted key must verify");
}

#[test]
fn test_verify_against_key_wrong_key() {
    let secret = make_secret(42);
    ...
    let result = verify_against_key(&encoded, data, &wrong);
...
#[test]
fn test_verify_chain_against_key_correct() {
    let secret = make_secret(42);
    let tk = trusted_key(42);
    ...
    let result = verify_chain_against_key(
        &child_enc, child_art,
...
#[test]
fn test_verify_chain_against_key_wrong_child_key() {
...
#[test]
fn test_verify_chain_against_key_wrong_parent_key() {
