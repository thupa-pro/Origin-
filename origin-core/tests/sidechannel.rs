...
fn make_tampered_poo(poo: &ProofOfOrigin) -> ProofOfOrigin {
    ...
    // Flip the MSB of hash[0] - changes the commitment
    tampered.hash[0] ^= 0x80;
    tampered
}
...
fn measure_verify(poo: &ProofOfOrigin, artifact: &[u8], iterations: usize) -> Vec<f64> {
    ...
    for _ in 0..iterations {
        ...
        let _ = verify_statement(&stmt, artifact);
...
fn test_timing_side_channel_t_test() {
    ...
    let samples_valid = measure_verify(&poo, &artifact, 50_000);
    let samples_invalid = measure_verify(&tampered, &artifact, 50_000);
    ...
    // p > 0.05 threshold: |t| < ~1.96 for large df
    assert!(
        t_stat < 2.0,
        "Timing leak detected! t-statistic = {:.4}, mean_valid = {:.3}µs, mean_invalid = {:.3}µs",
        t_stat,
        m1 * 1_000_000.0,
        m2 * 1_000_000.0
    );

    eprintln!("=== Timing Side-Channel Analysis ===");
    eprintln!(
        "Valid   verify: mean = {:.3}µs, σ = {:.3}µs",
        m1 * 1_000_000.0,
        s1 * 1_000_000.0
    );
    eprintln!(
        "Invalid verify: mean = {:.3}µs, σ = {:.3}µs",
        m2 * 1_000_000.0,
        s2 * 1_000_000.0
    );
    eprintln!("Welch's t = {:.4} (gate: < 2.0 for p > 0.05)", t_stat);
}
...
fn test_tamper_flags_byte0() {
    ...
    // Flip flags byte 0 (reserved[0])
    poo.reserved[0] ^= 0x01;
    // Note: reserved must be zero, so this makes from_bytes fail
    let bytes = poo.to_bytes();
    let result = ProofOfOrigin::from_bytes(&bytes);
    assert!(
        result.is_err(),
        "reserved[0] non-zero MUST be rejected by from_bytes"
    );
}
