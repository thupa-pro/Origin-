...
use crate::{MediaFormat, embed, extract, EmbeddingConfig};
...
fn make_payload(version: u8) -> Vec<u8> {
    ...
    bytes[50] = 208; bytes[51] = 90; bytes[52] = 152; bytes[53] = 1;
    bytes[54] = 130; bytes[55] = 177; bytes[56] = 10; bytes[57] = 183;
    bytes[58] = 213; bytes[59] = 75; bytes[60] = 254; bytes[61] = 211;
    bytes[62] = 201; bytes[63] = 100; bytes[64] = 7; bytes[65] = 58;
    bytes[66] = 14; bytes[67] = 225; bytes[68] = 114; bytes[69] = 243;
    bytes[70] = 218; bytes[71] = 162; bytes[72] = 38; bytes[73] = 53;
    bytes[74] = 175; bytes[75] = 2; bytes[76] = 26; bytes[77] = 104;
    bytes[78] = 247; bytes[79] = 7; bytes[80] = 81; bytes[81] = 26;
    bytes
}
...
fn test_jpeg_embed_extract_roundtrip() {
    ...
    let config = EmbeddingConfig { format: MediaFormat::Jpeg, overwrite: false };
    let embedded = embed(&payload, MINIMAL_JPEG, &config).expect("embed should succeed");

    assert!(embedded.len() > MINIMAL_JPEG.len(), "embedded JPEG should be larger");
...
fn test_jpeg_extract_from_nonexistent_returns_none() {
    assert!(extract(MINIMAL_JPEG).is_none(), "no payload in minimal JPEG");
}
...
fn test_jpeg_overwrite_existing() {
    ...
    let config1 = EmbeddingConfig { format: MediaFormat::Jpeg, overwrite: false };
    let once = embed(&payload1, MINIMAL_JPEG, &config1).expect("first embed");

    // Without overwrite, should error
    let config2_no = EmbeddingConfig { format: MediaFormat::Jpeg, overwrite: false };
    assert!(embed(&payload2, &once, &config2_no).is_err(), "should reject without overwrite");

    // With overwrite, should succeed
    let config2_yes = EmbeddingConfig { format: MediaFormat::Jpeg, overwrite: true };
...
fn test_png_embed_extract_roundtrip() {
    ...
    let config = EmbeddingConfig { format: MediaFormat::Png, overwrite: false };
...
fn test_png_overwrite_existing() {
    ...
    let config1 = EmbeddingConfig { format: MediaFormat::Png, overwrite: false };
    let once = embed(&payload1, &png, &config1).expect("first embed");

    let config2_no = EmbeddingConfig { format: MediaFormat::Png, overwrite: false };
    assert!(embed(&payload2, &once, &config2_no).is_err(), "should reject without overwrite");

    let config2_yes = EmbeddingConfig { format: MediaFormat::Png, overwrite: true };
    let twice = embed(&payload2, &once, &config2_yes).expect("overwrite should succeed");
    let extracted = extract(&twice).expect("extract after overwrite");
    assert_eq!(extracted, payload2, "extracted should be the new payload");
}
...
fn test_mp3_embed_extract_roundtrip() {
    ...
    let config = EmbeddingConfig { format: MediaFormat::Mp3, overwrite: false };
...
fn test_mp3_overwrite_existing() {
    ...
    let config1 = EmbeddingConfig { format: MediaFormat::Mp3, overwrite: false };
    let once = embed(&payload1, &mp3, &config1).expect("first embed");

    let config2_no = EmbeddingConfig { format: MediaFormat::Mp3, overwrite: false };
    assert!(embed(&payload2, &once, &config2_no).is_err(), "should reject without overwrite");

    let config2_yes = EmbeddingConfig { format: MediaFormat::Mp3, overwrite: true };
    let twice = embed(&payload2, &once, &config2_yes).expect("overwrite should succeed");
    let extracted = extract(&twice).expect("extract after overwrite");
    assert_eq!(extracted, payload2, "extracted should be the new payload");
}
...
fn test_pdf_embed_extract_roundtrip() {
    ...
    let config = EmbeddingConfig { format: MediaFormat::Pdf, overwrite: false };
...
fn test_pdf_overwrite_existing() {
    ...
    let config1 = EmbeddingConfig { format: MediaFormat::Pdf, overwrite: false };
    let once = embed(&payload1, &pdf, &config1).expect("first embed");

    // With overwrite=true, should succeed (we just append more)
    let config2_yes = EmbeddingConfig { format: MediaFormat::Pdf, overwrite: true };
    let twice = embed(&payload2, &once, &config2_yes).expect("overwrite should succeed");
...
fn test_extract_wrong_format_returns_none() {
    let payload = make_payload(0x01);
    let jpeg_config = EmbeddingConfig { format: MediaFormat::Jpeg, overwrite: false };
